...
#     identifier "app.liquiddesktop.mac" and certificate leaf = H"..."
...
NAME="Liquid Desktop Local Signing"
...
openssl req -newkey rsa:2048 -nodes \
    ...
    -subj "/CN=$NAME/O=Liquid Desktop/C=US" \
...
openssl pkcs12 -export -out "$WORK/identity.p12" \
    ...
    -name "$NAME" -passout pass:liquiddesktop \
    -keypbe PBE-SHA1-3DES -certpbe PBE-SHA1-3DES -macalg sha1 2>/dev/null

# -T lets codesign use the private key without prompting for the keychain.
security import "$WORK/identity.p12" -k "$KEYCHAIN" -P liquiddesktop -T /usr/bin/codesign
