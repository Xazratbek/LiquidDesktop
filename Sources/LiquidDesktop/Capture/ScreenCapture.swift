...
final class ScreenCapture: NSObject, SCStreamOutput, SCStreamDelegate {
    ...
    private let queue = DispatchQueue(label: "app.liquiddesktop.capture", qos: .userInteractive)
    ...
    /// Starts (or restarts) capturing a display without this app's own
    /// windows, so the liquid refracts the real desktop rather than itself.
    func start(on displayID: CGDirectDisplayID, scale: Double, frameRate: Int32) async throws {
        ...
        let backing = Double(Self.backingScale(for: displayID))
        let config = SCStreamConfiguration()
        config.width = max(Int(Double(display.width) * backing * scale), 64)
        config.height = max(Int(Double(display.height) * backing * scale), 64)
        config.pixelFormat = kCVPixelFormatType_32BGRA
        config.colorSpaceName = Self.colorSpaceName(for: displayID)
        // The system draws the real pointer above the overlay; a captured copy
        // would show up a second time, refracted under the water.
